...
public class EdmondsKarpAlgorithm {
    public static int run(EscapeRouteNetwork n, String startNodeId, String endNodeId) {
        ...
        for (Edge e : restcapacityNetwork.getEdges()) {
            reverseEdges.add(e.reverse());
        }

        for (Edge e : reverseEdges) {
            restcapacityNetwork.addEdge(e);
        }
        ...
        while (true) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (Edge e : restcapacityNetwork.getEdges()) {
                    if (e.getStartNode().equals(current) && path.get(e.getStartNode()) == null
                            && !e.getEndNode().equals(start) && e.capacity() > e.flow()) {
                        path.put(current, e);
    ...
    /**
     * <p>
     * 1 procedure BFS(G, root) is 2 let Q be a queue 3 label root as discovered 4
     * Q.enqueue(root) 5 while Q is not empty do 6 v := Q.dequeue() 7 if v is the
     * goal then 8 return v 9 for all edges from v to w in G.adjacentEdges(v) do 10
     * if w is not labeled as discovered then 11 label w as discovered 12
     * Q.enqueue(w)
     * </p>
     * 
     * @param g
